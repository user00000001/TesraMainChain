...
import (
	...
	"github.com/TesraSupernet/TesraMainChain/common/mclock"
	...
	"github.com/TesraSupernet/TesraMainChain/metrics"
	"github.com/TesraSupernet/TesraMainChain/params"
	"github.com/TesraSupernet/TesraMainChain/pos/posconfig"
	posUtil "github.com/TesraSupernet/TesraMainChain/pos/util"
...
func (bc *BlockChain) WriteBlockAndState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	epid, slotId := posUtil.CalEpochSlotID(block.Time().Uint64())
	...
	//if bc.currentBlock.NumberU64() == 0 || block.NumberU64() > bc.currentBlock.NumberU64() {
	if (!bc.config.IsPosActive && (externTd.Cmp(localTd) > 0 || (externTd.Cmp(localTd) == 0 && mrand.Float64() < 0.5))) ||
		(bc.config.IsPosActive && (bc.currentBlock.NumberU64() == 0 || block.NumberU64() > bc.currentBlock.NumberU64())) {
...
func (st *insertStats) report(chain []*types.Block, index int) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		context := []interface{}{
			...
			"blockElapsed", time.Now().Unix() - int64(st.blockTime),
		}
		if st.queued > 0 {
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if ethash, ok := bc.engine.(*ethash.Ethash); ok {
		log.Trace("tesramainchain willing revert")
		err := ethash.VerifyPPOWReorg(bc, oldBlock, oldChain, newChain)
		if err != nil {
			log.Error("tesramainchain revert invalid")
			return err
		}
	}
