...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light TesraMainChain protocol")
	...
	log.Info("Light TesraMainChain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	p.Log().Debug("Light TesraMainChain peer connected", "name", p.Name())
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light TesraMainChain peer registration failed", "err", err)
		return err
	}
	defer func() {
...
func (pc *peerConnection) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, to uint64) error {
	reqID := genReqID()
	rq := &distReq{
		getCost: func(dp distPeer) uint64 {
			peer := dp.(*peer)
		...
		request: func(dp distPeer) func() {
			...
			cost := peer.GetRequestCost(GetBlockHeadersMsg, amount)
			peer.fcServer.QueueRequest(reqID, cost)
			return func() { peer.RequestHeadersByNumber(reqID, cost, origin, amount, skip, reverse, to) }
		},
	}
	_, ok := <-pc.manager.reqDist.queue(rq)
	if !ok {
