...
#
# The cluster contains a bootnode, a tesramain node and multiple swarm nodes, with
...
# static tesramain configuration
...
# start_bootnode starts a bootnode which is used to bootstrap the tesramain and
# swarm nodes
start_bootnode() {
...
# start_tesramain_node starts a tesramain node with --datadir pointing at <base-dir>/tesramain
# and a single, unlocked account with password "tesramain"
start_tesramain_node() {
  local dir="${base_dir}/tesramain"
  mkdir -p "${dir}"

  local password="tesramain"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating tesramain account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get tesramain account address"
  fi
  ...
  start_node "tesramain" "${GETH_IP}" "$(which tesramain)" ${args[@]}
}
...
create_account() {
  ...
  tesramain --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
