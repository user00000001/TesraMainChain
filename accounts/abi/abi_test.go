...
func TestInputVariableInputLength(t *testing.T) {
	...
	{ "type" : "function", "name" : "bytes32", "constant" : true, "inputs" : [ { "name" : "str", "type" : "bytes32" } ] },
	{ "type" : "function", "name" : "bytes32A", "constant" : true, "inputs" : [ { "name" : "str", "type" : "bytes32[]" } ] },
	...
	// test one string
	strin := "hello world"
	mybytes, err := rlp.EncodeToBytes(strin)
	...
	var a string
	err = abi.Unpack(&a, "strOne", mybytes)
	...
	// test bytes32
	bytes32in := [32]byte{1,2}
	bytes32out := [32]byte{0}
	btspack, err = abi.Pack("bytes32", bytes32in)
	err = abi.Unpack(&bytes32out, "bytes32", btspack)

	// test bytes32[]
	bytes32Ain := [][32]byte{{1,2},{2,3}}
	bytes32Aout := [][32]byte{{0},{0}}
	btspack, err = abi.Pack("bytes32A", bytes32Ain)
	err = abi.Unpack(&bytes32Aout, "bytes32A", btspack[4:])
